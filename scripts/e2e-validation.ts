...
async function main() {
  await logPhase("Phase 1: System Discovery", async () => {
    ...
    return {
      ...
      entities: tables,
      authFlow: "JWT in HTTP-only Cookie",
    };
  });
  ...
  await logPhase("Phase 3: Authentication Audit", async () => {
    // Test Valid Admin
    const adminRes = await request("/auth/login", "POST", { email: "admin@greenmeadows.demo", password: "Admin@123" });
    if (adminRes.status !== 200) throw new Error("Admin login failed");
    adminToken = adminRes.data.token || (adminRes.headers?.get?.("set-cookie")?.match(/authToken=([^;]+)/)?.[1]);
    
    // Test Valid Resident
    const resRes = await request("/auth/login", "POST", { phone: "A101", password: "Demo@123" });
    if (resRes.status !== 200) throw new Error("Resident login failed");
    residentToken = resRes.data.token;
    
    const meRes = await request("/me", "GET", undefined, residentToken);
    residentFlatId = meRes.data.flatId;
    societyId = meRes.data.societyId;

    // Test Invalid Login
    const invRes = await request("/auth/login", "POST", { phone: "A101", password: "WrongPassword" });
  ...
  await logPhase("Phase 4: Core Business Flow Validation", async () => {
    ...
    const availRes = await request(`/availability?start=${encodeURIComponent(startStr)}&end=${encodeURIComponent(endStr)}`, "GET", undefined, residentToken);
    if (!availRes.data.available) throw new Error("Vehicle not available for core flow");

    // 3. Create booking
    const bookRes = await request("/bookings", "POST", { start: startStr, end: endStr, vehicleId }, residentToken);
    if (bookRes.status !== 200) throw new Error(`Booking failed: ${JSON.stringify(bookRes.data)}`);
  ...
  await logPhase("Phase 6: Concurrency Testing", async () => {
    ...
    const promises = [];
    for (let i = 0; i < 20; i++) {
      promises.push(request("/bookings", "POST", { start: startStr, end: endStr, vehicleId }, residentToken));
    }
  ...
  await logPhase("Phase 7: Business Rule Validation", async () => {
    // Past booking
    const pastRes = await request("/bookings", "POST", { start: "2020-01-01T10:00:00Z", end: "2020-01-01T12:00:00Z", vehicleId }, residentToken);
    if (pastRes.status !== 400) throw new Error("Allowed past booking");

    // 15 min increment
    const futureRes = await request("/bookings", "POST", { start: "2030-01-01T10:15:00Z", end: "2030-01-01T12:00:00Z", vehicleId }, residentToken);
    if (futureRes.status !== 400) throw new Error("Allowed 15 min increment");
  ...
  await logPhase("Phase 8: Security Validation", async () => {
    ...
    if (idorRes.status !== 403) throw new Error("Resident accessed admin route!");
    
    // SQL Injection attempt in login
    const sqlRes = await request("/auth/login", "POST", { email: "admin' OR '1'='1", password: "pwd" });
    if (sqlRes.status === 200) throw new Error("SQL injection succeeded!");
    
    return { idorPrevented: true, sqlInjectionPrevented: true };
  });
